...
def get_env_parameters(cfg: DictConfig, byterl):
    ...
    fixed["battle_agent"] = agents.GreedyBattleAgent()
    
    # 3. Evaluation environments parameters
    evals = [
        fixed.copy(),  # vs. OSL (Greedy)
        {
            **common_params,
            "deck_building_agents": (byterl, agents.NoisyByteRL(temperature=0.5)),
            "battle_agent": agents.NoisyByteRL(temperature=0.5),
        },  # vs. 0.5-NByteRL
        {
            **common_params,
            "battle_agent": byterl,
...
def main(cfg: DictConfig):
    ...
    # Get env settings
    self_play_env_params, fixed_adversary_env_params, eval_env_params = get_env_parameters(cfg, byterl)
