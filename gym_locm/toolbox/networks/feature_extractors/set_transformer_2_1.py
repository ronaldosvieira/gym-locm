...
class SetTransformer21FeaturesExtractor(LOCMFeaturesExtractor):
    def __init__(
        self, 
    ...
    def forward(self, observations) -> dict[str, th.Tensor]:
        ...
        ), dim=1)  # [bs, 1+1+4+8+1+1+1+1+3+3+3+3, 32]
        
        all_entities = self.global_embedding(global_input)  # [bs, 30, 128]
        state = self.state_embedding(all_entities).squeeze(1)  # [bs, 128]

        return dict(
            hand_cards=all_entities[:, 6:14, :],  # [bs, 8, 128]
            p_lane0_creatures=all_entities[:, 18:21, :],  # [bs, 3, 128]
            p_lane1_creatures=all_entities[:, 21:24, :],
            op_lane0_creatures=all_entities[:, 24:27, :],
            op_lane1_creatures=all_entities[:, 27:30, :],
            p_lane0=all_entities[:, 14, :],  # [bs, 128]
            p_lane1=all_entities[:, 15, :],  # [bs, 128]
            state=state,  # [bs, 128]
