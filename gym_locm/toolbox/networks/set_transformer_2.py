...
class SetTransformerFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations) -> dict[str, th.Tensor]:
        ...
        p = th.cat((p, p.new_tensor(player).expand(p.size(0), -1)), dim=1)  # [bs, player_emb_dim + 8]
        op = th.cat((op, op.new_tensor(opponent).expand(op.size(0), -1)), dim=1)  # [bs, player_emb_dim + 8]
        ...
        ), dim=2)  # [bs, max_hand_size, card_emb_dim + 8]
        ...
        ), dim=2)  # [bs, max_lane_size, creature_emb_dim + 8]
        ...
        ), dim=1)  # [bs, 1+1+8+1+3+3+3+3, 32+8]
        state = self.global_embedding(state_input)  # [bs, 1+1+8+1+3+3+3+3, 64]
...
class SetTransformerLOCMNetwork(nn.Module):
    def __init__(
        ...
        self.lane_embedding = PMA(dim=64, num_heads=4, num_seeds=1)

        self.summon_source_card = nn.Linear(64, hidden_dim)
        self.summon_target_lane = nn.Linear(64, hidden_dim)
        ...
        self.use_source_card = nn.Linear(64, hidden_dim)
        self.use_target_creature = nn.Linear(64, hidden_dim)
        ...
        self.attack_source_creature = nn.Linear(64, hidden_dim)
        self.attack_target_creature = nn.Linear(64, hidden_dim)
    ...
    def forward_actor(self, embeddings: dict) -> th.Tensor:
        ...
        p = embeddings["state"][:, 0, :]  # [bs, 64]
        op = embeddings["state"][:, 1, :]  # [bs, 64]
        p_deck = embeddings["state"][:, 2, :]  # [bs, 64]
        p_hand = embeddings["state"][:, 3:11, :]  # [bs, max_hand_size, 64]
        p_lane0_creatures = embeddings["state"][:, 11:14, :]  # [bs, max_lane_size, 64]
        p_lane1_creatures = embeddings["state"][:, 14:17, :]  # [bs, max_lane_size, 64]
        op_lane0_creatures = embeddings["state"][:, 17:20, :]  # [bs, max_lane_size, 64]
        op_lane1_creatures = embeddings["state"][:, 20:23, :]  # [bs, max_lane_size, 64]

        # PASS logit
        pass_logit = self.pass_action(embeddings["state"]).squeeze(1)  # [bs, 1]
    ...
    def forward_critic(self, embeddings: dict) -> th.Tensor:
        return self.value_net(embeddings["state"]).squeeze(1)
