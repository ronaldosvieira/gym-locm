...
Precompiled binary wheels containing compiled Cython extensions are provided on PyPI for Windows, Linux, and macOS. When installing from source, the installation will attempt to compile Cython extensions if a C compiler is available locally, falling back to optimized pure Python mode otherwise.
